...
(defn db-select-singles
  ...
  (let [latency-rtt 10  ; Network round-trip time between server and client
    ...
    (for [id items]
      (do (Thread/sleep (+ latency-rtt latency-1))
...
(defn db-select-batch
  ...
  (let [latency-rtt 10  ; Network round-trip time between server and client
    ...
    (for [id items] {:id id :seed (/ (* id id) Math/PI)})))
...
(defn run-group
  ...
  (let [g (g-start)]
    (map deref
      (doall (for [items (range size)]
               (d/chain (grouper/submit! g items) mapping-fn))))))


#_(run-group 500 (fn [item] (assoc item :uuid (java.util.UUID/randomUUID))))

(with-open [g (g-start)] (grouper/submit! g 1))

#_(time (doall (db-select-singles (range 500))))
